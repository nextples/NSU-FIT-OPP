...
int main(int argc, char** argv) {
    ...
    double prev_proc_max_diff = /*EPSILON*/  1;
    double max_diff = 0.0;
...
//    MPI_Request reduce_max_diff_req;
    ...
    layer_heights = (int*) malloc(sizeof(int) * proc_count);
    offsets = (int*) malloc(sizeof(int) * proc_count);
    divide_area_into_layers(layer_heights, offsets, proc_count);

    prev_func = (double*) malloc(sizeof(double) * layer_heights[proc_rank] * N_Y * N_Z);
    curr_func = (double*) malloc(sizeof(double) * layer_heights[proc_rank] * N_Y * N_Z);
    init_layers(prev_func, curr_func, layer_heights[proc_rank], offsets[proc_rank]);

    up_border_layer = (double*) malloc(sizeof(double) * N_Y * N_Z);
    down_border_layer = (double*) malloc(sizeof(double) * N_Y * N_Z);
    ...
    do {
        ...
        tmp_max_diff_2 = calc_border(prev_func, curr_func, up_border_layer, down_border_layer, layer_heights[proc_rank], offsets[proc_rank], proc_rank, proc_count);
...
//        MPI_Wait(&reduce_max_diff_req, MPI_STATUS_IGNORE);
        ...
        MPI_Allreduce(&prev_proc_max_diff, &max_diff, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    ...
    for (int i = 0; i < layer_height; i++) {
        for (int j = 0; j < N_Y; j++) {
            for (int k = 0; k < N_Z; k++) {
                ...
                if (isBorder) {
                    double tmp = phi(get_x(offset + i), get_y(j), get_z(k));
                    prev_func[get_index(i, j, k)] = tmp;
                    curr_func[get_index(i, j, k)] = tmp;
                }
                else {
    ...
    for (int i = 1; i < layer_height - 1; i++) {
        for (int j = 1; j < N_Y - 1; j++) {
            for (int k = 1; k < N_Z - 1; k++) {
                ...
                curr_func[get_index(i, j, k)] = (f_i + f_j + f_k - rho(get_x(offset + i), get_y(j), get_z(k))) / (2 / H_X_2 + 2 / H_Y_2 + 2 / H_Z_2 + A);
    ...
    for (int j = 1; j < N_Y - 1; j++)
        for (int k = 1; k < N_Z - 1; k++) {
            if (proc_rank != 0) {
                ...
                curr_func[get_index(0, j, k)] = (f_i + f_j + f_k - rho(get_x(offset), get_y(j), get_z(k))) / (2 / H_X_2 + 2 / H_Y_2 + 2 / H_Z_2 + A);
            ...
            if (proc_rank != proc_count - 1) {
                ...
                curr_func[get_index(layer_height - 1, j, k)] = (f_i + f_j + f_k - rho(get_x(offset + layer_height - 1), get_y(j), get_z(k))) / (2 / H_X_2 + 2 / H_Y_2 + 2 / H_Z_2 + A);
